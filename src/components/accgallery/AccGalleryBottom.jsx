...
export function AccGalleryBottom() {
  ...
  useEffect(() => {
    axios
      .get(`${apiBaseUrl}/findstay/acc/gallery/${accid}/bottom`)
      .then((result) => {
          ...
          <div className="gallery_frame" key={index}>
            ...
            <div className="gallery_img room_img">
              <img
                ...
                alt={`${room.room_name}이미지1`}
              />
              <img
                src={getImgPath.room(room.room_img2)}
                alt={`${room.room_name}이미지2`}
              />
              <img
                src={getImgPath.room(room.room_img3)}
                alt={`${room.room_name}이미지3`}
              />
            </div>
          </div>
        ))}
