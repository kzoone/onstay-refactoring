...
export function AccMap() {
  ...
  }, [accid]); // accid가 변경될 때마다 useEffect가 실행됨

  useEffect(() => {
    if (accMap) {
      const container = document.getElementById("map");
      const options = {
        center: new kakao.maps.LatLng(accMap.latitude, accMap.longitude),
        level: 3,
      };
  ...
  }, [accMap]); // accMap이 변경될 때마다 useEffect가 실행됨

  return (
    <div className="map_container">
      <div className="explain">
        {accMap.acc_name}의 위치는 {accMap.address}입니다.
      </div>
